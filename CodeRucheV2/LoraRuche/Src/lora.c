...
/* Déclarations externes (Définies dans l'application principale) */
...
/* Allocation des tampons de communication USART */
char rx_buffer_lora[128]; // Tampon de réception (Rx)
char cmd_buffer[128];     // Tampon de formatage des commandes AT (Tx)

/* Identifiants d'approvisionnement The Things Network (TTN) - Méthode OTAA */
...
// Évalue la présence d'un code d'erreur dans la trame de retour du modem
...
void LoRa_Setup_And_Join(void) {
    /* 1. Configuration des paramètres physiques MAC (Délais de scrutation optimisés à 300ms) */
    ...
    if (strstr(rx_buffer_lora, "Network joined") == NULL) {
        // Échec d'attachement : verrouillage de sécurité de l'application
        while(1); 
    }
}
