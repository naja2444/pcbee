...
/* Base de temps dédiée aux capteurs synchrones critiques */
...
void Timer_Init_Microsecondes(void) {
    ...
    // Objectif : Obtenir une résolution temporelle de 1 µs (1 MHz) depuis une source système à 16 MHz.
    // Algorithme : PSC = (F_Systeme / F_Cible) - 1 = (16000000 / 1000000) - 1 = 15
    TIM3->PSC = 15;

    // 3. Configuration de la limite de comptage (Auto-Reload Register)
    // Maximisation de la plage d'itération (16-bit, soit 65535 µs avant événement d'Update/Débordement)
    TIM3->ARR = 0xFFFF; 
...
void Delai_us(uint32_t us) {
    ...
    // Attente active (Polling) jusqu'à ce que le différentiel temporel atteigne la cible fixée en paramètre
    // Note d'ingénierie : Le transtypage explicite en (uint16_t) gère intrinsèquement les cas de débordement (Overflow) du timer
